...
app.get('/', function (req, res){
    return res.send({ error: true,message: 'Test Student Wen API'})
});
...
app.post('/std', function(req,res){
    ...
    dbConn.query("INSERT INTO student SET ?", std , function(error, results, fieldds){
        if (error) throw error;
        return res.send(results);
    });
});
...
app.put('/std/:id', function(req, res){
    ...
    dbConn.query("UPDATE student SET ? WHERE std_id =?",[std, std_id],function(error,results,fields){
        if (error) throw error;
        return res.send({error: false,data: results, message:'Student has been update successfully.'});
    });
});
...
app.delete('/std/:id',function(req,res){
    ...
    dbConn.query('DELETE FROM student WHERE std_id = ?',std_id,function(error ,results,fields){
        if (error) throw error;
        return res.send({error: false,data: results,message:'Student has been delete successfully.'});
    });
});
